...
func (sr *ScoreRenderer) RenderNote(note Note, x, y float32) {
	...
	if note.StaffLine < 0 {
		...
		for line := 0; line > note.StaffLine; line -= 2 {
			if line <= 0 {
				rl.DrawLineEx(
					rl.NewVector2(centerX-StaffSpacesToPixels(0.75), ledgerY-StaffSpacesToPixels(float32(line)*0.5)),
					rl.NewVector2(centerX+StaffSpacesToPixels(0.75), ledgerY-StaffSpacesToPixels(float32(line)*0.5)),
					thickness, rl.Black)
			}
		}
...
func midiToFreq(m int) float64 {
	return 440.0 * math.Pow(2, (float64(m)-69.0)/12.0)
}
...
func (sr *ScoreRenderer) updatePlayback() {
	...
	for i, ev := range sr.schedule {
		if t >= ev.start && t < ev.end { idx = i; break }
	}
	sr.activeIndex = idx
	// audio
	if !sr.audioReady { return }
...
func GetNoteStaffPosition(midiNote int) int {
	...
	switch midiNote {
	case 60: return -2  // C4 - ledger line below staff
	case 62: return -1  // D4 - below staff
	case 64: return 0   // E4 - bottom staff line
	case 65: return 0   // F4 - between bottom and second line
	case 67: return 1   // G4 - second line
	case 69: return 2   // A4 - between second and third line  
	case 71: return 3   // B4 - third line (middle)
	case 72: return 3   // C5 - between third and fourth line
	case 74: return 4   // D5 - fourth line
	case 76: return 5   // E5 - top staff line
	case 77: return 5   // F5 - between top line and above
	case 79: return 6   // G5 - above staff
	default:
